...
from app import db
from app.models import BKL_Product, BKL_CartItem
...
def index():
    try:
        products = BKL_Product.query.filter(BKL_Product.stock > 0, BKL_Product.available == True).all()
        cart = get_cart()
...
def details(id):
    try:
        product = BKL_Product.query.get_or_404(id)
        cart = get_cart()
